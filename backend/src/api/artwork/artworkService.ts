...
import { extractUsageInfo, generateArtworkStory } from "@/common/services/openai";
import { logger } from "@/server";

type ArtworkUpdateData = Parameters<ArtworkRepository["update"]>[1];
...
export class ArtworkService {
	...
	async findAll() {
		try {
			...
			if (!artworks || artworks.length === 0) {
				return ServiceResponse.failure("No artworks found", null, StatusCodes.NOT_FOUND);
			}
	...
	async findById(id: string) {
		try {
			...
			if (!artwork) {
				return ServiceResponse.failure("Artwork not found", null, StatusCodes.NOT_FOUND);
			}
			...
			logger.error(`Error finding artwork with id ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
	...
	async findByArtist(artistId: string) {
		try {
			...
			if (!artist) {
				return ServiceResponse.failure("Artist not found", null, StatusCodes.NOT_FOUND);
			}

			const artworks = await this.artworkRepository.findByArtist(artistId);
			return ServiceResponse.success("Artworks found", artworks);
		} catch (ex) {
			logger.error(`Error finding artworks by artist: ${(ex as Error).message}`);
			return ServiceResponse.failure(
	...
	async create(data: {
		...
		try {
			...
			if (!resolvedArtistId && data.artistName) {
				const artist = await this.artistRepository.findOrCreate(data.artistName, {
			...
			if (resolvedArtistId) {
				const existing = await this.artworkRepository.findByTitleAndArtist(data.title, resolvedArtistId);
				if (existing) {
			...
			/**
			 * Build the Prisma payload explicitly so only persisted artwork fields are sent.
			 * That keeps helper inputs like artistName out of the write and makes it clear
			 * that canonical museum assignment is not part of the public create flow.
			 */
			...
			return ServiceResponse.success("Artwork created", artwork, StatusCodes.CREATED);
		} catch (ex) {
	...
	async update(
		...
		try {
			...
			const updateData: ArtworkUpdateData = { ...data };
			...
			logger.error(`Error updating artwork with id ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
	...
	async delete(id: string) {
		try {
			...
			logger.error(`Error deleting artwork with id ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
	...
	async generateStory(id: string, userId: string) {
		try {
			...
			// ── Cooldown check ──
			const latestLog = await this.aiUsageLogRepository.findLatestForArtwork(userId, id, ArtworkService.STORY_ENDPOINT);
			...
			const todayCount = await this.aiUsageLogRepository.countSince(userId, ArtworkService.STORY_ENDPOINT, startOfDay);
			...
			if (todayCount >= dailyLimit) {
				return ServiceResponse.failure(
					`Daily story limit reached (${dailyLimit}/${dailyLimit}). ${
						plan === "FREE" ? "Upgrade your plan for more generations." : "Limit resets at midnight."
					}`,
					{
						used: todayCount,
						limit: dailyLimit,
			...
			const isTrusted = artwork.verified || trustedSources.includes(artwork.source);
			...
			logger.error(`Error generating story for artwork ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
