...
export class SavedArtworkService {
	...
	async findByUser(userId: string) {
		try {
			const savedArtworks = await this.savedArtworkRepository.findByUser(userId);
			return ServiceResponse.success("Saved artworks found", savedArtworks);
		} catch (ex) {
			logger.error(`Error finding saved artworks for user ${userId}: ${(ex as Error).message}`);
	...
	async findByCollection(collectionId: string, userId: string) {
		try {
			...
			if (!collection || collection.userId !== userId) {
				return ServiceResponse.failure("Collection not found", null, StatusCodes.NOT_FOUND);
			}

			const savedArtworks = await this.savedArtworkRepository.findByCollection(collectionId, userId);
			return ServiceResponse.success("Saved artworks found", savedArtworks);
		} catch (ex) {
			logger.error(`Error finding saved artworks in collection ${collectionId}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
				"An error occurred while retrieving saved artworks.",
				null,
				StatusCodes.INTERNAL_SERVER_ERROR,
	...
	async findById(id: string, userId: string) {
		try {
			...
			if (!savedArtwork) {
				return ServiceResponse.failure("Saved artwork not found", null, StatusCodes.NOT_FOUND);
			}
			...
			logger.error(`Error finding saved artwork ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
	...
	async save(
		...
		try {
			const artworkExistsPromise = data.artworkId
				? this.artworkRepository.exists(data.artworkId)
			...
			if (!resolvedCollectionId) {
				const [defaultCollection, artworkExists] = await Promise.all([
					this.collectionRepository.findDefaultByUser(userId),
					artworkExistsPromise,
				]);
				...
				const [collection, artworkExists] = await Promise.all([
					this.collectionRepository.findOwnershipById(resolvedCollectionId),
					artworkExistsPromise,
				]);
			...
			return ServiceResponse.success("Artwork saved", savedArtwork, StatusCodes.CREATED);
		} catch (ex) {
	...
	async update(
		...
		try {
			...
			if (data.collectionId && data.collectionId !== existing.collectionId) {
				const targetCollection = await this.collectionRepository.findOwnershipById(data.collectionId);
				if (!targetCollection || targetCollection.userId !== userId) {
					return ServiceResponse.failure("Target collection not found", null, StatusCodes.NOT_FOUND);
				}
			}
			...
			logger.error(`Error updating saved artwork ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
	...
	async remove(id: string, userId: string) {
		try {
			...
			logger.error(`Error removing saved artwork ${id}: ${(ex as Error).message}`);
			return ServiceResponse.failure(
